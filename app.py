...
# تخصيص المظهر باللغة العربية والخطوط الأنيقة
st.markdown("""
<style>
...
groq_key = st.sidebar.text_input("مفتاح Groq API Key:", value=st.secrets.get("GROQ_API_KEY", ""), type="password")
gemini_key = st.sidebar.text_input("مفتاح Google Gemini Key:", value=st.secrets.get("GEMINI_API_KEY", ""), type="password")
s2_key = st.sidebar.text_input("مفتاح Semantic Scholar (اختياري):", value=st.secrets.get("S2_API_KEY", ""), type="password")
...
def crawl_academic_platforms(query):
    papers = []
